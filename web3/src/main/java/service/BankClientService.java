...
public class BankClientService {
    ...
    public boolean sendMoneyToClient(BankClient sender, String name, Long value) {
        try {
            BankClientDAO dao = getBankClientDAO();
            BankClient nameTo = dao.getClientByName(name);
            if (nameTo != null && dao.validateClient(sender.getName(), sender.getPassword()) && dao.isClientHasSum(sender.getName(), value)) {
                getPostresqlConnection().setAutoCommit(false);
                dao.updateClientsMoney(sender.getName(), sender.getPassword(), -value);
                dao.updateClientsMoney(name, nameTo.getPassword(), value);
                getPostresqlConnection().commit();
                return true;
            } else return false;
        } catch (SQLException e) {
            try {
                getPostresqlConnection().rollback();
                getPostresqlConnection().setAutoCommit(true);
    ...
    private static Connection getPostresqlConnection() {
        try {
    ...
    private static BankClientDAO getBankClientDAO() {
        return new BankClientDAO(getPostresqlConnection());
    }
}
